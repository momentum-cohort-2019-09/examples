...
function main() {
  ...
  searchForm.addEventListener('submit', function (event) {
    ...
    var searchType = formData.get('search-type');
    var searchTerm = formData.get('search-term');
...
function search(type, name) {
  getResults("https://swapi.co/api/".concat(type, "/?search=").concat(name), []);
}
...
function populateResults(results) {
  ...
  var listItems = results.map(function (p) {
    return p.name;
  }).map(function (name) {
    return "<li>".concat(name, "</li>");
  });
  resultsDiv.innerHTML = "<ul>".concat(listItems.join('\n'), "</ul>");
}
